...
function install() {
    ...
    git clone \
        ...
        || (echo "Failed to clone Supra Aptos Core" && exit 2)
    cd aptos-core
    ...
    npm install || (echo "Failed to initialize the Supra Aptos TypeScript SDK" && exit 3)
    npm run build || (echo "Failed to build the Supra Aptos TypeScript SDK" && exit 4)
    cd "$SCRIPT_ROOT"
